...
if(isset($_POST['order'])){
    ...
    $order_query = mysqli_query($con, "SELECT * FROM `orders` WHERE name = '$name' AND number = '$number' AND email = '$email' AND method = '$method' AND address = '$address' AND total_product = '$total_products' AND total_price = '$cart_total'") or die('query failed');
    ...
    if($cart_total == 0){
        ...
        mysqli_query($con, "INSERT INTO `orders`(`user_id`, `name`, `number`, `email`, `method`,
         `address`, `total_product`, `total_price`, `placed_on`) VALUES('$user_id', 
         '$name', '$number', '$email', '$method', '$address', '$total_products', '$cart_total', 
